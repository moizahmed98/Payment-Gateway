...
import authorizeNetCardAuthentication from '@salesforce/apex/CardRegistrationController.authorizeNetCardAuthentication';
...
@track currentSelectedTab="Global Payments";
...
//This function formats the CVV input, adds a forward slash at the third position if needed
handleCVVChange(event) {
    ...
    // Update the CVV with the formatted number
    else
    {
        this.outputCVV = limitedInput;
    }
}

//This function handles card submission, updates the request indicator, and triggers different toast events
handleSubmitClick(){
    console.log('Current Selected::'+this.currentSelectedTab);

    //Fetch the values of input card details
    ...
    //Invalid or incomplete Card CVV
    else if(cardCVVInput == '' || cardCVVInput == null || cardCVVInput.length != 3 )
    {
        const toastRequestRecievedErrorEvent = new ShowToastEvent({
            title: 'Error!',
    ...
    //Invalid or incomplete Card Expiry
    ...
    {
        if(this.currentSelectedTab == 'Global Payments')
        {
            authorizeNetCardAuthentication({  cardNumber: cardNumberInput,  cardExpirationDate: cardExpiryInput,  cardCVV: cardCVVInput  })
            .then(result => {
